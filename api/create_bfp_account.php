...
$insertSql = "INSERT INTO users (fullname, email, password, role, phone, address) VALUES ('$fullname', '$email', '$hashedPassword', '$role', '$phone', '$address')";
if ($conn->query($insertSql) === TRUE) {
    $new_user_id = $conn->insert_id;

    // Create notifications for all admin users about new BFP account creation
    $adminQuery = "SELECT id FROM users WHERE role IN ('admin') AND is_active = 1";
    $adminResult = $conn->query($adminQuery);

    if ($adminResult && $adminResult->num_rows > 0) {
        $notifyStmt = $conn->prepare("INSERT INTO notifications (user_id, title, body) VALUES (?, ?, ?)");
        $title = "New BFP Personnel Account Created";
        $body = "A new BFP personnel account has been created for '$fullname' ($email).";
